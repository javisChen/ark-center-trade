...
@Validated
@RestController
@RequestMapping("/v1/uni-pay")
@RequiredArgsConstructor
public class UniPayTradeController extends BaseController implements PayApi {
    ...
    public SingleResponse<PayOrderCreateDTO> createPayOrder(@RequestBody @Validated PayOrderCreateCommand command) {
        return SingleResponse.ok(payOrderCommandHandler.pay(command));
    }

    @Operation(summary = "退款")
    @PostMapping("/refund")
    public SingleResponse<PayOrderCreateDTO> refund(@RequestBody @Validated PayOrderCreateCommand command) {
        return SingleResponse.ok(payOrderCommandHandler.refund(command));
    }
